...
//            for (float theta = 0.0f; theta < (2 * (float)Math.PI); theta += 0.01f)
//            {
//                GL.Vertex3(
//                    (float)Math.Cos(theta) * radius + keypoint.position.x * scale,
//                    (float)Math.Sin(theta) * radius + keypoint.position.y * scale, 0f);
//            }
//            GL.End();
//            GL.PopMatrix();
//        }
