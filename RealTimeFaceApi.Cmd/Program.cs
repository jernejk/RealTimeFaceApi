...
public static class Program
{
    ...
    {
        _faceClient = new FaceClient(new ApiKeyServiceClientCredentials(Constants.FaceSubscriptionKey))
        {
            Endpoint = "https://westus.api.cognitive.microsoft.com"
        };

        if (string.IsNullOrWhiteSpace(Constants.FaceSubscriptionKey))
        {
            ...
            using (Mat image = new Mat())
            {
                while (true)
                {
                    ...
                    if (hasChange)
                    {
                        ...
                        if (_faceRecognitionTask == null && !string.IsNullOrWhiteSpace(Constants.FaceSubscriptionKey))
                        {
    ...
    private static async Task StartRecognizing(Mat image)
    {
        try
        {
            ...
            if (faceIds.Any())
            {
                ...
                var potentialUsers = await _faceClient.Face.IdentifyAsync(faceIds, Constants.FaceGroupId);
    ...
    private async static Task<string> GetCandidateName(Guid? personId)
    {
        ...
        {
            _cachedIdentities = await _faceClient.PersonGroupPerson.ListAsync(Constants.FaceGroupId);
        }
